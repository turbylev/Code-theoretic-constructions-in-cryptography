...
В реализации перестановка хранится как вектор `perm`, а равенство проверяется как `A*G1 == G2[:, perm]` (все операции по модулю 2). См. реализацию `isequivSSP`.
...
4. Дальше перебираются перестановки внутри классов и для каждого кандидата проверяется, существует ли матрица `A`, удовлетворяющая `A*G1 == G2[:,perm]`.

### Зачем я ввёл лимит перебора
Если сигнатура слабая, классы выходят большими, и перебор перестановок становится факториальным.  
Поэтому в код добавлен параметр `max_tries` — ограничение на число попыток, после чего алгоритм прекращает поиск и возвращает специальный код результата `-2`.
...
Реализация: `sig2_dim_hull_short(G, i)`.
...
Реализация: `sig3_ws_hull_short(G, i)`.
...
Файл с реализацией алгоритма и вспомогательной линейной алгебры над GF(2): `isequivSSP.jl`.   
...
- `-2` — если достигнут лимит перебора `max_tries`.

### 4.2 Вспомогательные
- `gf2_mul(A,B)` — умножение матриц над GF(2).
- `rref_gf2`, `rank_gf2` — Гаусс/ранг над GF(2).
- `shortened_generator(G,i)` — порождающая матрица укороченного кода `C_{\{i\}}`.
- `hull_generator(G)` — порождающая матрица `Hull(C)=C∩C^⊥`.
- `weight_spectrum_from_generator(H)` — спектр весов кода, заданного порождающей матрицей `H`.
...
## 7) Набор тестов корректности

Чтобы убедиться, что реализация SSA работает правильно, добавлен отдельный файл с тестами: `tests_task02.jl`.

### Что именно проверяют тесты
Тесты построены так, чтобы покрыть основные сценарии из определения эквивалентности кодов (существование `A` и `P`) и крайние случаи:

1. **test_perm_only** — эквивалентность только за счёт перестановки столбцов `P`  
   Берётся небольшая матрица `G1`, строится `G2 = G1[:, perm]`.  
   Ожидается, что алгоритм найдёт `(A, perm)` и выполнится проверка `A*G1 == G2*P`.

2. **test_A_only** — эквивалентность только за счёт обратимой матрицы строковых преобразований `A`  
   Берётся матрица `G1` полного ранга и обратимая `A`, строится `G2 = A*G1`.  
   Ожидается успешное нахождение эквивалентности и выполнение равенства.

3. **test_A_and_perm_random** — общий случай: `A + P` одновременно   
   При фиксированном `seed` генерируются `G1`, обратимая `A` и `perm`, затем `G2 = A*G1[:, perm]`.  
   Ожидается успешный результат и проверка равенства.

4. **test_not_equiv_by_rank** — заведомо неэквивалентный случай  
   Строятся `G1` и `G2` с разными рангами (разной размерностью порождаемого кода), что исключает эквивалентность.  
   Ожидается `-1`.

5. **test_limit** — корректная обработка ограничителя `max_tries`  
   Для эквивалентной пары запускается SSA с очень маленьким `max_tries`, чтобы алгоритм быстро завершался и не зависал.  
   Ожидается возврат `-2`.
...
```bash
julia tests_task02.jl
```

Если всё корректно, в консоли будет последовательность сообщений `OK: ...` и финальная строка `Все тесты пройдены`.

## 7) Результаты

**Обозначения статуса**
- `OK` — эквивалентность найдена в пределах лимитов.
- `LIMIT` — достигнут лимит перебора `max_tries` (возврат `-2`).
